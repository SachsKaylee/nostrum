defmodule Nostrum.Putter do
  ...
  - Automatically generates a `@doc` if not defined.
    - Examples for the autogenerated @doc can be supplied via `@doc_example`.
    - An example can have a literal value. In this case the example will be assumed to be as-in as-out.
    - A more complex example can be supplied via `%Nostrum.Putter.Example{}`. The `values` key is a proplist of named arguments, the `result` is the value that will ultimately assigned be to `field` in this example.
